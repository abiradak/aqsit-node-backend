...
exports.update = (req, res, next) => {
    ...
    if (categoery_id) {
        ...
        jobsCategoery.findByIdAndUpdate(categoery_id, { $set: dynamicQuery })
            .then((result) => {
...
exports.delete = (req, res, next)=>{
    ...
    if (categoery_id) {
        jobsCategoery.findByIdAndDelete(categoery_id)
            .then((result) => {
...
exports.fetchforEstimate = (req, res, next) => {
    var user_id = req.query.user_id;
    if (user_id) {
        jobsCategoery.find({ user_id: user_id })
            .then((result) => {
                if (result.length) {
                    res.send({ message: "Data found", status: true, data: result });
                } else {
...
function jobsCategoeryAssignments() {
    async.eachSeries(config.jobsCategoery,(eachdesign,next)=>{
        jobsCategoery.update({categoery : eachdesign.categoery},{$set : {categoery : eachdesign.categoery}},{upsert : true},(error,result)=>{
                // if (error) {
